...
class TestTRFBasic(unittest.TestCase):
    ...
    def test_linear_least_squares(self):
        ...
        popt, pcov = curve_fit(linear, x, y, method='trf')
        self.assertAlmostEqual(popt[0], 2.0, places=1)
        self.assertAlmostEqual(popt[1], 1.0, places=0)  # Slightly looser tolerance for intercept
...
class TestTRFSpecialCases(unittest.TestCase):
    ...
    def test_max_iterations(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            popt, pcov = curve_fit(
                slow_converging_model, x, y,
                p0=[1, 0.05, 1, 1],
                method='trf',
                max_nfev=5  # Very low limit
            )
        # Check that the error message indicates max iterations exceeded
        self.assertIn("maximum number of function evaluations", str(context.exception).lower())
