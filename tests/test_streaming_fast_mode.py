...
class TestFastModeConfiguration:
    ...
    def test_fast_mode_skips_retry_attempts(self):
        ...
        assert result['streaming_diagnostics']['batch_success_rate'] < 1.0, "Should have at least one failed batch"
    ...
    def test_fast_mode_logs_errors_to_stderr(self):
        ...
        assert result['streaming_diagnostics']['batch_success_rate'] < 1.0, "Should have at least one failed batch"
        assert result['streaming_diagnostics']['batch_success_rate'] > 0.0, "Should have at least one successful batch"
    ...
    def test_fast_mode_explicit_disable(self):
        ...
        assert config.enable_fault_tolerance == False
...
class TestFastModeVsFullMode:
    ...
    def test_fast_mode_omits_diagnostics_full_mode_includes(self):
        ...
        # Fast mode still includes diagnostics but they're minimal
        assert len(result_fast['streaming_diagnostics']['recent_batch_stats']) == 0  # fast
    ...
    def test_fast_mode_overhead_benchmark(self):
        ...
        config_warmup = StreamingConfig(batch_size=100, max_epochs=1, enable_checkpoints=False)
        ...
        overhead_percent = ((avg_full - avg_fast) / avg_fast) * 100 if avg_fast > 0 else 0
