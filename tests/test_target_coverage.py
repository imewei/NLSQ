...
from nlsq.minpack import curve_fit, CurveFit
...
class TestTargetCoverage(unittest.TestCase):
    ...
    def test_curve_fit_errors(self):
        ...
        try:
            popt, pcov = curve_fit(bad_model, x, y, p0=[1, 2])  # b=2 causes division by zero at x=2
        ...
        try:
            popt, pcov = curve_fit(bad_model, x, y, p0=[1, 0], bounds=([2, 1], [1, 2]))
    ...
    def test_curve_fit_with_jacobian_sparsity(self):
        ...
        popt, pcov = curve_fit(sparse_model, x, y, p0=[1, 1, 10])
    ...
    def test_bounds_edge_cases(self):
        ...
        popt, pcov = curve_fit(model, x, y, bounds=([-np.inf, -1], [np.inf, 1]))
        self.assertAlmostEqual(popt[0], 2.0, places=3)
    ...
    def test_robust_fitting(self):
        ...
        for loss in ['soft_l1', 'huber', 'cauchy', 'arctan']:
            popt, pcov = curve_fit(model, x, y, loss=loss, f_scale=1.0)
    ...
    def test_convergence_criteria(self):
        ...
        popt, pcov = curve_fit(model, x, y, ftol=0.1, xtol=0.1, gtol=0.1)
