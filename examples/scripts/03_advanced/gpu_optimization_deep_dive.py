...
start = time.time()
cf_new.curve_fit(
    exponential_model, x_200, y_200, p0=[2.0, 0.3]
)  # Different shape!
...
cf_gpu = CurveFit()
_ = cf_gpu.curve_fit(complex_model, x_large[:100], y_large[:100], p0=[3, 0.5, 2, 1])
...
if has_gpu:
    ...
    print(f"  Expected speedup vs CPU: 5-20x for this problem size")
else:
    print(
...
print(f"   Example: {large_array.shape} array uses {get_array_memory_mb(large_array):.1f} MB")
print()
...
def benchmark_nlsq(n_points_list, n_params=2, n_runs=5):
    ...
    for n_points in n_points_list:
        ...
        for _ in range(n_runs):
            ...
            # Ensure completion (JAX is async)
            popt[0].block_until_ready()
            times.append((time.time() - start) * 1000)
...
ax2.loglog(
    bench_results["n_points"], bench_results["mean_time_ms"], "o-", label="NLSQ"
)
