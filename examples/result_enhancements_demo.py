...
def example3_confidence_intervals():
    ...
    for i, (val, ci95, ci99) in enumerate(zip(result.popt, ci_95, ci_99)):
        print(f"p{i:<11} {val:>8.4f}    [{ci95[0]:>7.4f}, {ci95[1]:>7.4f}]    "
...
def example4_prediction_intervals():
    ...
    print(f"  " + "-" * 50)
    for i in range(5):
        print(f"  {x[i]:.2f}    {y[i]:>6.3f}    {result.predictions[i]:>6.3f}    "
    ...
    for i, x_val in enumerate(x_new):
        ...
        print(f"  {x_val:.2f}     {y_pred:>6.3f}    {pi_new[i, 0]:>6.3f}    "
...
def example5_visualization():
    ...
    print(f"  ✓ Plot saved to 'curve_fit_result.png'")
    ...
    print(f"  ✓ Plot saved to 'curve_fit_result_no_residuals.png'")
...
def example7_model_comparison():
    ...
    for name, res in models:
        print(f"{name:<15} {len(res.popt):<8} {res.r_squared:<10.6f} "
    ...
    print(f"✓ Exponential model has the lowest AIC/BIC (correct model)")
...
def example8_residuals_predictions():
    ...
    print(f"\nVerification:")
    print(f"  Residuals match (data - predictions)? "
