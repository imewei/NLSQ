...
def second_order_step_response(t, K, zeta, omega_n, t_delay):
    ...
    response = K * (1 - jnp.exp(-zeta * omega_n * t_eff) *
                    jnp.cos(omega_d * t_eff - phi) / jnp.cos(phi))
    return jnp.where(t >= t_delay, response, 0.0)
...
print(f"  Residuals normal? p = {p_value_normality:.3f} " +
...
ax1.errorbar(time, output_measured, yerr=sigma, fmt='o', alpha=0.4,
             markersize=3, capsize=0, label='Measured data')
t_fine = np.linspace(0, 100, 500)
ax1.plot(t_fine, first_order_step_response(t_fine, K_true, tau_true, t_delay_true),
         'r--', linewidth=2, label='True system', alpha=0.7)
ax1.plot(t_fine, first_order_step_response(t_fine, *popt),
         'g-', linewidth=2.5, label='Fitted model')

# Mark key points
ax1.axhline(K_fit * 0.632, color='blue', linestyle=':', alpha=0.5)
ax1.axvline(t_63, color='blue', linestyle=':', alpha=0.5,
            label=f'63.2% at t={t_63:.1f}s')
ax1.axhline(K_fit, color='gray', linestyle='--', alpha=0.5,
            label=f'Steady-state: {K_fit:.1f}°C')
...
ax3.plot(x_norm, np.exp(-x_norm ** 2 / 2) / np.sqrt(2 * np.pi),
...
ax5.plot(time[mask_rise], output_measured[mask_rise], 'o', alpha=0.5,
         markersize=4, label='Data')
ax5.plot(t_fine[:250], first_order_step_response(t_fine[:250], *popt),
         'g-', linewidth=2, label='Fitted model')
