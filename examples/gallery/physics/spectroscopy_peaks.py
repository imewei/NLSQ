...
def multi_peak_model(x, bg_slope, bg_offset,
                     amp1, cen1, width1,  # Peak 1 (Gaussian)
                     amp2, cen2, width2,  # Peak 2 (Gaussian)
                     amp3, cen3, width3):  # Peak 3 (Lorentzian)
    """
...
spectrum_true = multi_peak_model(
    energy,
    bg_slope_true, bg_offset_true,
    amp1_true, cen1_true, width1_true,
    amp2_true, cen2_true, width2_true,
    amp3_true, cen3_true, width3_true
)
...
bounds = (
    [0, 0,  # background
     0, 6, 0.1,  # peak 1: amp > 0, 6 < cen < 9, width > 0.1
     0, 7, 0.1,  # peak 2: amp > 0, 7 < cen < 10, width > 0.1
     0, 9, 0.1],  # peak 3: amp > 0, 9 < cen < 13, width > 0.1
    [10, 100,  # background
     2000, 9, 1.0,  # peak 1
     1000, 10, 1.0,  # peak 2
     500, 13, 1.0]  # peak 3
...
(bg_slope_fit, bg_offset_fit,
 amp1_fit, cen1_fit, width1_fit,
 amp2_fit, cen2_fit, width2_fit,
 amp3_fit, cen3_fit, width3_fit) = popt

# Parameter uncertainties
(bg_slope_err, bg_offset_err,
 amp1_err, cen1_err, width1_err,
 amp2_err, cen2_err, width2_err,
 amp3_err, cen3_err, width3_err) = perr
...
ax1.errorbar(energy, spectrum_measured, yerr=sigma, fmt='.',
             alpha=0.4, label='Measured spectrum', capsize=0,
             markersize=3)
ax1.plot(energy, multi_peak_model(energy, *popt), 'r-',
...
ax1.plot(energy, bg + peak1, 'g--', linewidth=1.5,
         label=f'Peak 1 ({cen1_fit:.2f} keV)')

peak2 = gaussian(energy, amp2_fit, cen2_fit, width2_fit)
ax1.plot(energy, bg + peak2, 'b--', linewidth=1.5,
         label=f'Peak 2 ({cen2_fit:.2f} keV)')

peak3 = lorentzian(energy, amp3_fit, cen3_fit, width3_fit)
ax1.plot(energy, bg + peak3, 'm--', linewidth=1.5,
         label=f'Peak 3 ({cen3_fit:.2f} keV)')
...
y_norm = len(normalized_residuals) * \
...
ax4.errorbar(energy[mask1], spectrum_measured[mask1],
             yerr=sigma[mask1], fmt='o', alpha=0.5,
             capsize=3, label='Data')
ax4.plot(energy[mask1], multi_peak_model(energy[mask1], *popt),
         'r-', linewidth=2, label='Fit')
ax4.plot(energy[mask1], bg[mask1], 'k--', linewidth=1.5, label='Background')
ax4.plot(energy[mask1], bg[mask1] + peak1[mask1], 'g--',
         linewidth=1.5, label='Peak 1')
...
ax5.errorbar(energy[mask2], spectrum_measured[mask2],
             yerr=sigma[mask2], fmt='o', alpha=0.5,
             capsize=3, label='Data')
ax5.plot(energy[mask2], multi_peak_model(energy[mask2], *popt),
         'r-', linewidth=2, label='Fit')
ax5.plot(energy[mask2], bg[mask2], 'k--', linewidth=1.5, label='Background')
ax5.plot(energy[mask2], bg[mask2] + peak2[mask2], 'b--',
         linewidth=1.5, label='Peak 2')
ax5.plot(energy[mask2], bg[mask2] + peak3[mask2], 'm--',
         linewidth=1.5, label='Peak 3')
ax5.set_xlabel('Energy (keV)')
ax5.set_ylabel('Counts')
ax5.set_title(f'Peaks 2 & 3 (Kβ + Escape)')
ax5.legend()
...
print(f"  Peak 1 (Kα):     {cen1_fit:.3f} ± {cen1_err:.3f} keV, " +
      f"area = {area1:.0f} counts·keV")
print(f"  Peak 2 (Kβ):     {cen2_fit:.3f} ± {cen2_err:.3f} keV, " +
      f"area = {area2:.0f} counts·keV")
print(f"  Peak 3 (Escape): {cen3_fit:.3f} ± {cen3_err:.3f} keV, " +
      f"area = {area3:.0f} counts·keV")
print(f"\nPeak intensity ratio (Kα/Kβ): {area1/area2:.2f}")
print(f"(Typical for many elements: ~1.5-2.5)")
