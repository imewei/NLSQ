...
t_half_2nd_err = t_half_2nd * np.sqrt((k_2nd_err/k_2nd_fit)**2 + (C0_2nd_err/C0_2nd_fit)**2)
...
print(f"  Note: For 2nd order, t₁/₂ = 1/(k[A]₀)")
...
print(f"Testing first-order data:")
print(f"  1st-order fit RMSE: {rmse_1st_model:.5f} M")
print(f"  2nd-order fit RMSE: {rmse_2nd_model:.5f} M")
print(f"  → Best fit: {'First-order' if rmse_1st_model < rmse_2nd_model else 'Second-order'}")
...
print(f"First-order linearization (ln[A] vs t):")
...
ax1.errorbar(time, C_1st_measured, yerr=sigma_1st, fmt='o', alpha=0.4,
             markersize=3, capsize=0, label='Measured')

t_fine = np.linspace(0, 1000, 200)
ax1.plot(t_fine, first_order_decay(t_fine, C0_1st, k_1st_true),
         'r--', linewidth=2, label='True', alpha=0.7)
ax1.plot(t_fine, first_order_decay(t_fine, *popt_1st),
         'g-', linewidth=2.5, label='Fitted')

ax1.axhline(C0_1st_fit / 2, color='orange', linestyle=':', alpha=0.5)
ax1.axvline(t_half_1st, color='orange', linestyle=':', alpha=0.5,
...
ax2.semilogy(time, C_1st_measured, 'o', alpha=0.4, markersize=3,
             label='Measured')
ax2.semilogy(t_fine, first_order_decay(t_fine, *popt_1st),
             'g-', linewidth=2.5, label='Fitted (exponential)')

# Linear fit to log-transformed data
ax2.semilogy(t_fine, np.exp(linear_1st_order(t_fine, *popt_ln)),
             'b--', linewidth=2, label='Linear fit (ln[A] vs t)')
...
ax4.errorbar(time, C_2nd_measured, yerr=sigma_2nd, fmt='o', alpha=0.4,
             markersize=3, capsize=0, label='Measured')
ax4.plot(t_fine, second_order_decay(t_fine, C0_2nd, k_2nd_true),
         'r--', linewidth=2, label='True', alpha=0.7)
ax4.plot(t_fine, second_order_decay(t_fine, *popt_2nd),
         'g-', linewidth=2.5, label='Fitted')

ax4.axhline(C0_2nd_fit / 2, color='orange', linestyle=':', alpha=0.5)
ax4.axvline(t_half_2nd, color='orange', linestyle=':', alpha=0.5,
...
ax5.plot(time, inv_C_2nd, 'o', alpha=0.4, markersize=3,
         label='1/[A] data')
ax5.plot(t_fine, linear_2nd_order(t_fine, *popt_inv),
         'b-', linewidth=2.5, label=f'Linear fit (slope={k_inv_fit:.4f})')
...
ax7.plot(time, C_1st_measured, 'o', alpha=0.5, markersize=4,
         label='1st-order data')
ax7.plot(t_fine, first_order_decay(t_fine, *popt_1st),
         'g-', linewidth=2.5, label='1st-order fit')
ax7.plot(t_fine, second_order_decay(t_fine, *popt_2nd_on_1st),
         'b--', linewidth=2, label='2nd-order fit (wrong model)')
...
ax8.plot(t_fine, rate_1st, 'g-', linewidth=2.5,
         label=f'1st-order: r = k[A]')
ax8.plot(t_fine, rate_2nd, 'b-', linewidth=2.5,
         label=f'2nd-order: r = k[A]²')
...
ax9.text(0.1, 0.9, table_text, fontsize=9, verticalalignment='top',
...
print(f"  Half-life:     {t_half_2nd:.2f} ± {t_half_2nd_err:.2f} s (at [A]₀ = {C0_2nd_fit:.2f} M)")
