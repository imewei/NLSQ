...
def run_partial_batch_benchmarks():
    ...
    for config in configs:
        ...
        warmup_config = StreamingConfig(batch_size=100, max_epochs=1, batch_shape_padding='auto')
        warmup_opt = StreamingOptimizer(warmup_config)
        _ = warmup_opt.fit((x_warmup, y_warmup), model_function, p0=[2.0, 0.2], verbose=0)
        ...
        print(f"  Batches: {result_dynamic['n_full_batches']} full + {result_dynamic['partial_batch_size']} points partial")
        ...
        print(f"  Max batch shape: {result_auto['batch_padding'].get('max_batch_shape', 'N/A')}")
        print(f"  Post-warmup recompiles: {result_auto['batch_padding'].get('post_warmup_recompiles', 'N/A')}")
    ...
    for i in range(0, len(results), 2):
        ...
        print(f"  Post-warmup recompiles (auto): {auto['batch_padding'].get('post_warmup_recompiles', 'N/A')}")
    ...
    print(f"On GPU, improvements of 30-50% are expected due to higher JIT overhead.")
