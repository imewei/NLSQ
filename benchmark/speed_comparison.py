...
from comparison_utils import extract_params, get_gpu_cpu_conversion, mod_container
...
class PlotResults:
    def plot_overall_speed(self, df_dict, dlengths, savefig=True):
        _fig, ax1 = plt.subplots(figsize=(5, 4))
        ...
        for key, ls, color, marker in zip(
            dkeys, lstyles, colors, markers, strict=False
        ):
            tdict = df_dict[key]
    ...
    def plot_operation_speed(self, df_dict, dlengths, conversion_time, savefig=True):
        def order_labels(ax, olabels):
            ...
            handle_dict = dict(zip(labels, handles, strict=False))
        ...
        legend_dict = dict(zip(all_time_keys, legend_labels, strict=False))
        ...
        colors = ["C0", "C1", "C2", "C3", "C4"]
        markers = ["o", "s", "^", "d", "+"]
        _fig, axes = plt.subplots(2, 1, figsize=(5, 8))

        ax1, ax2 = axes
        plabels = ["(a)", "(b)"]
        for ax, plabel in zip(axes, plabels, strict=False):
            ax.annotate(
                plabel,
                xy=(-0.215, 0.975),
        ...
        for time_key, color, marker in zip(time_keys1, colors, markers, strict=False):
            for key, ls in zip(dkeys, lstyles, strict=False):
                tdict = df_dict[key]
                ...
                ax1.plot(
                    dlengths,
                    ydata,
                    label=key + " " + legend_dict[time_key],
                    linestyle=ls,
                    color=color,
                    marker=marker,
                )
        ...
        ax1.legend(
            ohandles1, olabels1, ncol=2, frameon=True, loc="best", columnspacing=0.25
        )
        ...
        for time_key, color, marker in zip(time_keys2, colors, markers, strict=False):
            for key, ls in zip(dkeys, lstyles, strict=False):
                tdict = df_dict[key]
                ydata = extract_params(tdict, time_key, ptype="time_param")
                ydata = [np.mean(val) for val in ydata]
        ...
        ax2.legend(
            ohandles2, olabels2, ncol=2, frameon=True, loc="best", columnspacing=1
        )
...
def dict_results(results, index, fit_time):
    ...
    length_dict = dict(zip(iter_data_labels, iter_data, strict=False))
    return length_dict
...
for lindex, length in enumerate(lengths):
    ...
    for index, row in subdf.iterrows():
        ...
        st = time.time()
        jax_results = jcf.curve_fit(
            gaussian2d_jax, flat_XY, flat_data, p0=seed, **jax_kwargs
        )  # call gpu fitting func
        st2 = time.time()
        scipy_results = curve_fit(
            gaussian2d_np, flat_XY, flat_data, p0=seed, **scipy_kwargs
        )  # call gpu fitting func
...
df_dict = {"JAX": all_jax_dicts, "SciPy": all_scipy_dicts}  # , 'JAX_ut': jax_ut_df}
dlengths = lengths**2
