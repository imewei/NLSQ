...
def benchmark_mode(
    ...
    print(f"Mode: {mode}, Points: {n_points:,}, Batch: {batch_size}, Epochs: {max_epochs}")
    print(f"{'='*70}")
    ...
    _ = optimizer.fit((x_data[:100], y_data[:100]), model_function, p0=[2.0, 0.2], verbose=0)
    ...
    if batch_padding:
        ...
        print(f"  Post-warmup recompiles: {batch_padding.get('post_warmup_recompiles', 0)}")
...
def run_benchmarks():
    ...
    for config in configs:
        ...
        improvement_pct = (1 - result_auto['elapsed_time'] / result_dynamic['elapsed_time']) * 100
        ...
        print(f"  Target: 30-50% improvement")
        print(f"  Status: {'✓ PASS' if improvement_pct >= 15 else '✗ FAIL (below 15%)'}")
    ...
    for i in range(0, len(results), 2):
        ...
        print(f"  Recompiles (auto mode): {auto['batch_padding'].get('post_warmup_recompiles', 'N/A')}")

    avg_improvement = np.mean(improvements)
    print(f"\nAverage improvement: {avg_improvement:.1f}%")
    print(f"Target: 30-50% improvement")
    print(f"Overall status: {'✓ PASS' if avg_improvement >= 25 else '⚠ PARTIAL (some benefit but below target)'}")
