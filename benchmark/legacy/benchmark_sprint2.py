...
try:
    ...
    from nlsq.func_cache import cached_jit, get_function_cache
...
class PerformanceBenchmark:
    ...
    def run_all_benchmarks(self):
        ...
        print(
            f"Cache Hit Rate:              {cache_results['cache_hit_rate'] * 100:.1f}%"
        )
        print(f"Memory Pool Efficiency:      {pool_results['memory_mb']:.2f} MB pooled")
        print()
        print("Scaling (size → time):")
        for size, time_ms in zip(
            size_results["sizes"], size_results["times_ms"], strict=False
        ):
