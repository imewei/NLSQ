...
def sparse_function(x, a, b, c):
    ...
    THRESHOLD = 3  # Threshold value for switching between functions
    result = jnp.zeros_like(x)
    result = jnp.where(x < THRESHOLD, a * jnp.exp(-b * x), result)
    result = jnp.where(x >= THRESHOLD, c * x**2, result)
    return result
...
class BasicBenchmark:
    ...
    def run_1d_exponential(self, sizes: list[int] | None = None) -> dict:
    ...
    def run_2d_gaussian(self, sizes: list[int] | None = None) -> dict:
        ...
        for size in sizes:
            ...
            true_params = [
                1.5,  # amplitude
                self.rng.uniform(-2, 2),  # x0
                self.rng.uniform(-2, 2),  # y0
                1.0 + self.rng.random(),  # sigma_x
                1.0 + self.rng.random(),  # sigma_y
                self.rng.uniform(-np.pi / 4, np.pi / 4),  # theta
                0.1,  # offset
            ]

            # Generate data
            data = gaussian_2d(coords, *true_params)
            data += jnp.array(self.rng.normal(0, 0.05, data.shape))
        ...
        for solver in solvers:
            try:
                ...
                popt, _ = fitter.curve_fit(
                    exponential_model_jax, x, y, p0=[1.0, 1.0], solver=solver
                )
                ...
                print(
                    f"{solver:>10} {elapsed:12.2f} {popt[0]:8.3f} {popt[1]:8.3f} {error:10.4f}"
                )
...
class LargeDatasetTesting:
    ...
    def test_curve_fit_large(self, sizes: list[int] | None = None) -> dict:
    ...
    def test_memory_estimation(self) -> dict:
        ...
        print(
            f"{'Dataset':>15} {'Params':>8} {'Base (GB)':>12} {'Jacobian (GB)':>15} {'Total (GB)':>12}"
        )
        print("-" * 65)
        ...
        for n_data, n_params in test_cases:
            ...
            print(
                f"{n_data:15,} {n_params:8} {base_memory_gb:12.3f} "
...
class AdvancedFeatureTesting:
    ...
    def test_sparse_jacobian(self) -> dict:
        ...
        y = np.array(y) + self.rng.normal(0, 0.01, x.shape)
    ...
    def test_streaming_optimizer(self) -> dict:
        ...
        n_params = 2
        batch_memory_mb = (config.batch_size * n_params * 8 * 3) / (
            1024 * 1024
        )  # data, jacobian, work
...
Examples:
  ...
  python benchmark.py --suite solver --gpu
    ...
    parser.add_argument(
        "--gpu", action="store_true", help="Use GPU if available (default: CPU)"
    )
