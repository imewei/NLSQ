...
class GaussianParameters:
    ...
    def lab_widths(self, wx, wy, theta):
        ...
        wyy = ((wx * np.sin(theta)) ** 2 + (wy * np.cos(theta)) ** 2) ** 0.5
        return wxx, wyy
    ...
    def edge_mask(self, shape, wx, wy, theta, scale=2):
        ...
        box_coords = [round(val) for val in box_coords]
...
class SimulationParameters:
    ...
    def create_simulation_params(self):
        ...
        lengths = np.ndarray.astype(dlengths**0.5, dtype=int)
        row_datas = []
        counter = 0
        for length in lengths:
            ...
            for _i in range(num_samples):
                gparams, noise_std = gbp.get_single_gaussian_data(
                    X, Y, a_vals, w_vals, theta_vals, offset_vals, noise_vals, img_dims
                )
