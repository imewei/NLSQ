...
The NLSQ GUI provides an interactive graphical interface for performing nonlinear least squares curve fitting with GPU/TPU acceleration. This guide covers all features and workflows available in the application.
...
4. [Fitting Options](#fitting-options)
...
Navigate between pages using the sidebar. Pages become accessible as you complete earlier steps.
...
|--------|------------|-------------|
| CSV | `.csv` | Comma-separated values with optional header |
| ASCII | `.txt`, `.dat`, `.asc` | Whitespace or tab-delimited text files |
| NumPy | `.npz` | Compressed NumPy array files |
...
- **1D Curve**: Standard x-y data for 1D curve fitting
- **2D Surface**: Grid data (x, y, z) for surface fitting
...
|-------|----------|------------|
| Linear | y = ax + b | a, b |
| Exponential Decay | y = a * exp(-bx) + c | a, b, c |
| Exponential Growth | y = a * exp(bx) + c | a, b, c |
| Gaussian | y = A * exp(-(x-mu)^2/(2*sigma^2)) | A, mu, sigma |
| Lorentzian | y = A / (1 + ((x-x0)/gamma)^2) | A, x0, gamma |
| Sigmoid | y = L / (1 + exp(-k*(x-x0))) + b | L, k, x0, b |
...
- Degree 2: Quadratic (y = c0*x^2 + c1*x + c2)
- And so on...
...
**Important**: Use `jax.numpy` (or `jnp`) instead of standard `numpy` for GPU acceleration and automatic differentiation.
...
| Preset | Tolerance | Multi-start | Use Case |
|--------|-----------|-------------|----------|
| Fast | 1e-6 | Disabled | Quick exploratory fits |
| Robust | 1e-8 | 10 starts | Reliable production fits |
| Quality | 1e-10 | 20 starts | High-precision requirements |
...
| Adjusted R-squared | R-squared adjusted for number of parameters |
...
| Format | Description |
|--------|-------------|
| JSON | Full results with parameters, statistics, metadata |
| CSV | Parameter table with values and uncertainties |
| HTML | Interactive Plotly charts (self-contained) |
...
1. **Cold Start**: The first fit may take longer due to JIT compilation. Subsequent fits reuse compiled code.

2. **Large Datasets**: For datasets over 1M points, the streaming optimizer is automatically enabled.

3. **Memory**: Monitor memory usage for very large datasets. Consider using data chunking.

### Getting Help

- GitHub Issues: [https://github.com/imewei/NLSQ/issues](https://github.com/imewei/NLSQ/issues)
