...
- **FR4 — model_id required.** If `checkpoint_dir` is set and `model_id` is
  `None`, raise `ValueError`. The model function's closure cannot be safely
  fingerprinted (`co_code`/`co_consts` alone miss closure-cell values —
  confirmed in the three-brain review), so identity is the caller's
...
- **FR7 — Preemption signal.** `SIGTERM` and `SIGUSR1` handlers set a
  ...
  normal `max_generations`/`xtol` stop.
- **FR8 — Atomic writes.** Every checkpoint save writes to a temp file in the
  same directory, fsyncs, and `os.replace`s over the previous
  `{run_id}.h5`; the immediately-prior good file is rotated to
  `{run_id}.h5.bak` before replacement. `load()` falls back to `.bak` and
  logs a warning if the primary file is missing a completion marker or fails
  to open.
...
- [ ] FR1-FR8 each have at least one passing test (see plan for exact tests).
